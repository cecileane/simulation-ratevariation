...
```{r}
viz_violin = function(dat, ycolname="D3", H="H2"){
  ggplot(dat) +
  facet_grid(genedist~ lindist+linbygenedist+edgedist, labeller = label_both) +
  geom_violin(aes(x=factor(!!sym(H)), y=abs(!!sym(ycolname))), col = "blue")
}
```

H2 is a better predictor than H1, in the good scenario
without any rate variation (top left):

```{r}
viz_violin(concat_d3, H="H1")
viz_violin(concat_d3)
```

Original p-values appear to be uniformly distributed (as expected)
before Bonferroni correction for the 3 test per triplet
(top left, under the test assumption of no rate variation, and no reticulation):

```{r}
viz_violin(concat_d3, ycolname="P") +
  geom_hline(yintercept=0.05, linetype="dashed", color = "red", size=0.4)
```

## Visualizing Dstat results

same as with D3, perhaps a bit more pronounced with D:
H1=1 has small D values (won't be detected), but
H2=1 has larger values (will be detected )

```{r}
viz_violin(concat_dstat, ycolname="D", H="H1")
viz_violin(concat_dstat, ycolname="D")
```

p-values (no multiple comparison)

```{r}
viz_violin(concat_dstat, ycolname="P") +
  geom_hline(yintercept=0.05, linetype="dashed", color = "red", size=0.4)
```


## Visualizing HyDe results

```{r}
viz_violin(filtered_concat_hyde, ycolname="Pvalue", H="H1")
viz_violin(filtered_concat_hyde, ycolname="Pvalue")
```

# line plots for proportion of significant results

The proportion of "significant" results is the proportion of cases
when the p-value is below some threshold, like 0.05 or 0.01.
...
## logistic reg to quantify the effect of each type of rate variation
...
## line plots with proportion: type 1 error
...
```
```{r}
linD3= filter(prop_h1_h2, h2=="0" & h1!="0", test == "D3", lindist == "0.7") %>% group_by(simid,test) %>% groupprop
mean(linD3$prop_below_005)
```
...
get type-1 error rate when no rate variation for each test
```{r}
filter(prop_h1_h2, h2=="0" & h1!="0", simid=="1") %>% group_by(test) %>% groupprop
```
...
## line plots with proportion: power
